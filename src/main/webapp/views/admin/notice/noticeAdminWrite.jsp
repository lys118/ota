...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
               	<section id="notice_write">
					...
					<form id="writeForm" action="/notice/write" method="post">
						...
						<div style="display: flex; justify-content: space-between; margin-top:5px" >
							<input type="reset" id="rest_button" value="초기화">
    ...
    <%@include file="/views/admin/common/footer.jsp" %>
 	<script>
      ...
      function imageDelete(img){//이미지 삭제
    	let inputHidden;
      	if(typeof(img)=='object'){
    		...
    		inputHidden = document.getElementsByClassName(imgName);
      	}else{
      		inputHidden = document.getElementsByClassName(img);
      	}
      ...
      $('#rest_button').on('click',function(){//초시화 버튼 클릭시..
    	  $('#summernote').summernote('reset'); //textarea초기화
    	  getImageNameForDelete();//이미지 삭제
      });
      
      window.addEventListener("unload",unloadFunction);//새로고침,뒤로가기등 했을때...

      function unloadFunction(){
    	  getImageNameForDelete();
      }
            
      function getImageNameForDelete(){
    	  const imgNames = document.getElementsByName("newFileName"); 
    	  //names로 루프문이 부트스트랩+서머노트를 써서 에러가자꾸나서 아래방법으로함..
    	  let arr = [];
